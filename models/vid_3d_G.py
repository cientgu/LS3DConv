...
class vid_3d_G(BaseModel):
    ...
    def initialize(self, opt):
        ...
        self.netG = networks.define_G(conv_type=opt.G_conv_type, image_size=opt.loadSize, layer_num=9, model_type=opt.model, input_channel=35, if_sn=if_sn, padding_mode=opt.padding_mode, gpu_ids=opt.gpu_ids)
    ...
    def forward(self, input_A, encode_input=True):
    ...
    def inference(self, input_A, encode_input=True, save_mem=False):
        with torch.no_grad():
            self.netG.eval()
            if encode_input == True:
                input_A = self.encode_input(input_A) 
            if save_mem == True:
                ...
                fake_B = self.netG.upsample_model(temp)
            else:
                fake_B = self.netG(input_A)
        return fake_B
