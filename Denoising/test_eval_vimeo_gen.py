...
opt.dataset_mode = "video_denoise"
opt.model = "viddenoise"
...
def add_noise(tensors):
    size = tensors.size()
    # paepr noise
    noise1 = torch.zeros(size).data.normal_(0,25)*2/256
    # noise1 = (noise1>0).type(torch.FloatTensor)*noise1
    out1 = torch.clamp(tensors+noise1, -1, 1)
    return out1
    noise2 = torch.rand(size)*2-1
    tm = (torch.rand(size[0],1,size[2],size[3],size[4])>0.1).type(torch.FloatTensor).repeat(1,3,1,1,1)
    out2 = out1*tm + noise2*(1-tm)
    return out2
...
for idx, data in enumerate(dataset):        # data from temporal_dataset
    ...
    if opt.no_eval == False:
        psnr_gen = PSNR(groundtruth_image[3:4], output_image[3:4])
        ssim_gen = SSIM(groundtruth_image[3:4], output_image[3:4])
