...
class vid_inter_D(BaseModel):
    ...
    def forward(self, fake_B, input_B):
        ...
        # L1 loss
        loss_image = self.criterionL1(fa_gen[:,:,1:2,:,:], gt_gen[:,:,1:2,:,:])
        loss_image += self.criterionL1(fa_gen, gt_gen)
        ...
        if not self.opt.no_vgg:
            loss_G_VGG = 0
            loss_G_VGG += self.criterionVGG(self.reshape_tensor(fa_in), self.reshape_tensor(gt_in)) * self.opt.lambda_feat * 5
            loss_G_VGG += self.criterionVGG(self.reshape_tensor(fa_gen), self.reshape_tensor(gt_gen)) * self.opt.lambda_feat * 7
            loss_G_VGG += self.criterionVGG(self.reshape_tensor(fa_gen[:,:,1:2,:,:]), self.reshape_tensor(gt_gen[:,:,1:2,:,:])) * self.opt.lambda_feat * 2
