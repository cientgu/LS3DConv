...
class VideoDenoiseDataset(BaseDataset):
    ...
    def __getitem__(self, index):
        if self.opt.isTrain == True:
            ...
            B_paths = self.video_list[index % self.n_of_seqs].replace("\n","")
            # A_paths = B_paths.replace("input","target")    
            ...
            if self.opt.use_yuv == False:
                ...
                # A_img = Image.open(os.path.join(A_paths, "im4.png")).convert('RGB')
            else:
                B_img = Image.open(os.path.join(B_paths, "im%d.png"%(start_idx))).convert('YCbCr')
                # A_img = Image.open(os.path.join(A_paths, "im4.png")).convert('YCbCr')
            ...
            # Ai = self.get_image(os.path.join(A_paths, "im4.png"), transform_test_scale)
            # A = Ai.unsqueeze(1)
    ...
    def get_image(self, A_path, transform_scaleA, is_label=False):
        ...
        # A_img = transforms.functional.adjust_hue(A_img, 0.04*self.hue)
