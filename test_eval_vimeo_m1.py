...
for idx, data in enumerate(dataset):        # data from temporal_dataset
    ...
    with torch.no_grad():
        output_image1 = modelG(input_image)            # real_Bp is from input_B
        output_image2 = torch.flip(modelG(torch.flip(input_image, [4])),[4])
        output_image3 = torch.flip(modelG(torch.flip(input_image, [3])),[3])
        output_image4 = torch.flip(modelG(torch.flip(input_image, [3,4])),[3,4])
        # output_image = output_image1
        output_image = (output_image1 + output_image2 + output_image3 + output_image4)/4.0 
