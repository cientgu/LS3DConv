...
class SuperNet(nn.Module):
    def __init__(self,conv_type,image_size,block,layer_num,input_channel=3, if_sn=False, padding_mode="const"):
        ...
        pad1 = nn.ReplicationPad3d((1,1,1,1,1,1))
        pad2 = nn.ReplicationPad3d((1,1,1,1,1,1))
        deconv1 = nn.Conv3d(64,256,kernel_size=(3,3,3),stride=(1,1,1),padding=(0,0,0),bias=if_bias)
        deconv2 = DeformConv3dPack(256,1024,kernel_size=(3,3,3),stride=(1,1,1),padding=(0,0,0),bias=if_bias)
        relu1 = nn.ReLU()
        relu2 = nn.ReLU()
        bn1 = nn.BatchNorm3d(256)
        bn2 = nn.BatchNorm3d(1024)

        self.pixel_shuffle1 = nn.PixelShuffle(2)
        self.pixel_shuffle2 = nn.PixelShuffle(2)


        layer1.append(Res3DBlock(64,if_bias=if_bias, if_sn=if_sn, padding_mode=self.padding_mode, if_relu=False, conv_type="normal"))
        layer1.append(Res3DBlock(64,if_bias=if_bias, if_sn=if_sn, padding_mode=self.padding_mode, if_relu=False, conv_type="normal"))
        ...
        layer1.append(bn1)
        layer1.append(relu1)
        layer1.append(Res3DBlock(256,if_bias=if_bias, if_sn=if_sn, padding_mode=self.padding_mode, if_relu=False, conv_type=conv_type))
        layer1.append(pad2)
        layer1.append(deconv2)
        layer1.append(bn2)
        layer1.append(relu2)
        layer1.append(Res3DBlock(1024,if_bias=if_bias, if_sn=if_sn, padding_mode=self.padding_mode, if_relu=False, conv_type="normal"))
        ...
        layer2.append(nn.Conv3d(64,input_channel,kernel_size=(3,3,3),stride=(1,1,1),padding=(1,1,1),bias=if_bias))
    ...
    def forward(self, x, save_mem=False):
        ...
        out = self.conv1(x)
        out = self.layer1(out)
        b1,c1,t1,h1,w1 = out.size()
        out = self.pixel_shuffle1(out.permute(0,2,1,3,4).reshape(b1*t1,c1,h1,w1)).reshape(b1,t1,c1//4,h1*2,w1*2).permute(0,2,1,3,4)
        # out = self.layer2(out)
        b1,c1,t1,h1,w1 = out.size()
        out = self.pixel_shuffle2(out.permute(0,2,1,3,4).reshape(b1*t1,c1,h1,w1)).reshape(b1,t1,c1//4,h1*2,w1*2).permute(0,2,1,3,4)
        out = self.layer2(out)
        upsample_tensor = F.interpolate(x.reshape(b*c,t,h,w),scale_factor=4,mode='bilinear').reshape(b,c,t,4*h,4*w)
        out = out + upsample_tensor
        return out
