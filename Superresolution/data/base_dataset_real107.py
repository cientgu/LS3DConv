...
def get_transform(opt, params, method=Image.BICUBIC, normalize=True, toTensor=True):
    ...
    # transform_list += [transforms.adjust_gamma(0.3+0.5*random.random())]
    # transform_list += [transforms.functional.adjust_hue(0.02*(random.random()-0.5))]
    # transform_list += [transforms.functional.affine(5*rotate,[0,0],1+0.1*scale,3*shear,resample=Image.BICUBIC)]
...
def get_test_transform(opt, size):
    transform_list = []
    ...
    w, h = size
    new_w = int(round(w / 4)) * 4
    new_h = int(round(h / 4)) * 4    

    ### resize input image
    transform_list.append(transforms.Lambda(lambda img: __scale_image(img, (new_w, new_h), method)))
...
def get_video_params(opt, n_frames_total, cur_seq_len, index):
    #print("do some debug in get_video_params ++++++++++++++++++++")

    #print(n_frames_total)   # 4
    #print(cur_seq_len)      # 30
    #print(index)            # 14 or any other index

    tG = opt.n_frames_G     # 3
    if opt.isTrain:        
        n_frames_total = min(n_frames_total, cur_seq_len - tG + 1)     # 4

        n_gpus = opt.n_gpus_gen // opt.batchSize   #2 = 2//1               # number of generator GPUs for each batch
        n_frames_per_load = opt.max_frames_per_gpu * n_gpus # 2=2*1       # number of frames to load into GPUs at one time (for each batch)
        ...
        max_t_step = min(opt.max_t_step, (cur_seq_len-1) // (n_frames_total-1))   # still 1
        t_step = np.random.randint(max_t_step) + 1     # always be 1               # spacing between neighboring sampled frames
        offset_max = max(1, cur_seq_len - (n_frames_total-1)*t_step)  # maximum possible index for the first frame        
        #print("offset_max in base_dataset is: ")
        #print(offset_max)  # now is 25 = max(1,30-(6-1)*1)
