...
class vid_pred_D(BaseModel):
    ...
    def forward(self, fake_B, input_B):
        ...
        # L1 loss
        loss_image = self.criterionL2(fake_B, input_B)*0.2
        loss_image += (1 - self.criterionSSIM(fa_in, gt_in))*0.8
        ...
        if not self.opt.no_vgg:
            loss_G_VGG = 0
            loss_G_VGG += self.criterionVGG(self.reshape_tensor(fa_gen), self.reshape_tensor(gt_gen)) * self.opt.lambda_feat * 3
    ...
    def split_tensor(self,input):
        input_frames = self.opt.input_frames
        output_frames = self.opt.output_frames
        total_frames = input.size()[2]
        assert total_frames == input_frames + output_frames

        # frame_index = torch.zeros(input_frames).type(torch.cuda.LongTensor)
        # for index in range(input_frames):
        #     frame_index[index] = index

        # gen_index = torch.zeros(output_frames).type(torch.cuda.LongTensor)
        # for index in range(output_frames):
        #     gen_index[index] = index + opt.input_frames
        ...
        real_image = input[:,:,0:input_frames,:,:]
        gen_image = input[:,:,input_frames:total_frames,:,:]
