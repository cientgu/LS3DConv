...
class InterNet(nn.Module):
    ...
    def __init__(self,conv_type,image_size,block,layer_num,input_channel=3, if_sn=False, padding_mode="const"):
        ...
        deconv1 = DeconvUpBlock(128, if_sn=if_sn, if_relu=False)
        deconv2 = DeconvUpBlock(128, if_sn=if_sn, if_relu=False)
        deconv3 = DeconvUpBlock(128, if_sn=if_sn, if_relu=False)
        ...
        # resblock.append(Res3DBlock(256,if_bias=if_bias, if_sn=if_sn, padding_mode=self.padding_mode, if_relu=False, conv_type=conv_type))
        resblock.append(Res3DBlock(128,if_bias=if_bias, if_sn=if_sn, padding_mode=self.padding_mode, if_relu=False, conv_type=conv_type))
        ...
        deconv2_1 = nn.ConvTranspose3d(128,128,kernel_size=(3,4,4),stride=(1,2,2),padding=(1,1,1),bias=if_bias)
        bn2_1 = nn.BatchNorm3d(128)
        relu = nn.ReLU(inplace=True)


        deconv2_2 = nn.ConvTranspose3d(128,64,kernel_size=(3,4,4),stride=(1,2,2),padding=(1,1,1),bias=if_bias)

        bn2_2 = nn.BatchNorm3d(64)
        relu = nn.ReLU(inplace=True)

        conv2_2 = nn.Conv3d(64,32,kernel_size=(3,3,3),stride=(1,1,1),padding=(0,0,0),bias=if_bias)
        bn2_3 = nn.BatchNorm3d(32)
        if self.padding_mode == "shuyang":
            conv2_3 = nn.Conv3d(32,3,kernel_size=(1,7,7),stride=(1,1,1),padding=(0,0,0),bias=if_bias)
        else:
            conv2_3 = nn.Conv3d(32,3,kernel_size=(3,7,7),stride=(1,1,1),padding=(0,0,0),bias=if_bias)
        tanh = nn.Tanh()
