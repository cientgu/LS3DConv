                    ...
                    const int im2col_step)
{
    ...
    AT_ASSERTM(kernel_t_ == kernel_t && kernel_h_ == kernel_h && kernel_w_ == kernel_w,
               "Input shape and kernel shape wont match: (%d x %d x %d vs %d x %d x %d).", kernel_t, kernel_h, kernel_w, kernel_t_, kernel_h_, kernel_w_);
    ...
    const int time_out = (time + 2 * pad_t - (dilation_t * (kernel_t - 1) + 1)) / stride_t + 1;
    ...
    const int per_input_size = channels * time * height * width;
    const int per_offset_size = offset.size(1) * offset.size(2) * offset.size(3) * offset.size(4);
    const int per_mask_size = mask.size(1) * mask.size(2) * mask.size(3) * mask.size(4);
    auto output_n = output.view({batch/im2col_step_, batch_n * time_out * height_out * width_out, channels_out});
    for (int n = 0; n < batch/im2col_step_; ++n)
    {
        ...
        AT_DISPATCH_FLOATING_TYPES(input.type(), "deform_conv3d_forward_cuda", ([&] {
            deformable_3d_im2col_cuda(at::cuda::getCurrentCUDAStream(),
                                             ...
                                             batch_n, channels, time, height, width,
...
*/
    ...
    AT_ASSERTM(time_out == time_out_ && height_out == height_out_ && width_out == width_out_,
               "Input shape and grad_out shape wont match: (%d x %d x %d vs %d x %d x %d).", time_out, height_out, width_out, time_out_, height_out_, width_out_);

    auto ones = at::ones({batch * time_out * height_out * width_out}, input.options());
    auto columns = at::empty({channels * kernel_t * kernel_h * kernel_w, batch * 1 * time_out * height_out * width_out}, input.options());
    ...
    const int per_offset_size = offset.size(1) * offset.size(2) * offset.size(3) * offset.size(4);
    const int per_mask_size = mask.size(1) * mask.size(2) * mask.size(3) * mask.size(4);
    auto grad_output_n = grad_output.view({batch/im2col_step_, batch_n, channels_out, time_out, height_out, width_out});
//    printf("this is in the backward cuda function ---- \n");
